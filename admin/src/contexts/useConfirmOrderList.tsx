...
import { OrderContextProps, PaginationDetails } from './useNewOrders';

const ConfirmOrderContext = createContext<OrderContextProps | null>(null);

export const ConfirmOrderProvider = ({ children }: { children: ReactNode }) => {
  ...
  const column = [
    ...
    { key: 'deliveryDate', header: 'Delivery Date', width: '90px' },
  ];
  return (
...
export const useConfirmOrderContext = () => {
  const context = useContext(ConfirmOrderContext);
  if (!context) {
